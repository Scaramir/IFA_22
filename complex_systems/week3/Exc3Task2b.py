...
for num_sim in num_simulations:
    ...
    for i in range(1, num_sim+1):
        ...
        #  get the states at the integer time points
        states1 = states1[time_indices-1]
        states1_list_ssa.append(states1)
    # get the mean of states1_list_ssa for each time point
    states1_list_ssa = np.mean(states1_list_ssa, axis=0)
    all_x1_states_rk45 = integrate.solve_ivp(rhs, (0, t_final+step_size_of_int), initial_x_states.flatten(), method='RK45', t_eval=time_points).y[1]
    states1_list_rk45.extend(all_x1_states_rk45)
    conseccutive_sims.append(num_sim)
